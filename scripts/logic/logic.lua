...
function SRLocation.New(name)
	local self = setmetatable({}, SRLocation)
	if name then
		self.name = name
	else
		self.name = self
...
function SRLocation:connect_one_way(exit, rule, requiredExit)
	if type(exit) == "string" then
		exit = SRLocation.New(exit)
	end
	if rule == nil then
		rule = Always
	end
...
function SRLocation:connect_two_ways(exit, rule)
...
function SRLocation:connect_one_way_entrance(exit, rule, requiredExit)
	if rule == nil then
		rule = Always
	end
	self.exits[#self.exits + 1] = { exit, rule }
...
function SRLocation:connect_two_ways_entrance(exit, rule, requiredExit)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function SRLocation:connect_two_ways_entrance_door_stuck(exit, rule1, rule2)
...
function SRLocation:accessibility()
...
function SRLocation:discover(accessibility)
	local change = false
